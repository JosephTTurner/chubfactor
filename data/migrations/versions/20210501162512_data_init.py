...
from core.db_engine import db_session_scope
from core.models.base_model import Base
from data.models.color_model import Shade, Color
from data.models.brew_model import Brew, Style
from data.models.brewer_model import BrewCrew, Brewer, Crew
from data.models.chub_factor_model import ChubFactor
...
def upgrade():
    with db_session_scope() as db_session:
        ...
        stout = Style(name="Stout", shade=dark, color=chocolate)
        db_session.add(stout)
        coffeeless_stout = Style(
            name="Coffeeless Stout", shade=dark, color=chocolate, parent=stout
        )
        db_session.add(coffeeless_stout)
        irish_red_ale = Style(name="Irish Red Ale", shade=medium, color=red)
        db_session.add(irish_red_ale)
        american_red_lager = Style(
            name="American Red Lager", shade=medium, color=red, parent=irish_red_ale
        )
        ...
        brews = [
            ...
            Brew(name="Sass Dropper", brewer_id=isaac.id, style_id=saison.id),
            Brew(
                name="Indian Summer",
                brewer_id=joseph.id,
